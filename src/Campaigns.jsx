...
export default function Campaigns({ token, setToken, Frame, API, navigate, campaignId, create, postId }) {
  ...
  return <Frame token={token} setToken={setToken} eyebrow="Share the news" title={create ? "New blog email" : campaignId ? "Blog email" : "Email campaigns"}
    ...
    {!campaignId && !create && <><p>Share a published blog post with all subscribers or selected mailing lists. Preview and send a test before sending to your audience.</p><div className="admin-list">
      <div className="admin-list-heading"><h2>{campaigns.length} emails</h2><button className="text-button" onClick={() => refresh().catch(e => setError(e.message))}>Refresh</button></div>
    ...
    {saved && <section className="campaign-preview"><div className="admin-list-heading"><div><h2>{saved.status === "draft" ? "Review before sending" : saved.subject}</h2><p>{saved.preview?.recipient_count} recipients · {saved.status}</p></div><button className="text-button" disabled={busy || dirty} onClick={() => refresh().catch(e => setError(e.message))}>Refresh</button></div>
      ...
      {saved.status === "draft" && <button className="button button-primary" disabled={busy || dirty || !saved.preview?.recipient_count} onClick={send}>Send to {saved.preview?.recipient_count} subscribers</button>}
